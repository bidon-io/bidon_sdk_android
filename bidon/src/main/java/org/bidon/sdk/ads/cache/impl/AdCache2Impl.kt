...
internal class AdCache2Impl(
    ...
    override fun cache(adTypeParam: AdTypeParam) {
        ...
        job = scope.launch {
            ...
            while (true) {
                results.first { it.size < AdCache2.CacheItemToStartLoading }
                isLoading.first { !it }
    ...
    private fun load(adTypeParam: AdTypeParam) {
        ...
        if (results.value.size >= AdCache.CacheItemToStartLoading) {
        ...
        if (!isLoading.getAndUpdate { true }) {
            ...
            auction.start(
                ...
                onEach = { roundResults ->
                    scope.launch {
                        results.update {
                            resolver.sortWinners(it + roundResults).take(AdCache2.CacheCapacity)
                        }
